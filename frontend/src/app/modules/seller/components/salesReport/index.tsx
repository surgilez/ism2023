...
export const ListSellerReport = () => {
  ...
  const { promiseInProgress: promiseInProgressFiltering } = usePromiseTracker({
    area: 'salesSeller',
  })

  return (
    <div>
      {promiseInProgress || promiseInProgressFiltering ? (
                  ...
                  <table className="table">
                    ...
                    <tbody>
                      {membershipList?.map((membership, i) => {
                        const { name, lastName, doc, email, salesCurrenMont } =
                          membership

                        return (
                          salesCurrenMont &&
                          salesCurrenMont.length > 0 && (
                            <tr key={i}>
                              <td>
                                <input
                                  title="Check item"
                              ...
                              <td className="">
                                ...
                                salesCurrenMont.length > 0 ? (
                                  <i
                                    ...
                                    onClick={() => showSale(salesCurrenMont)}
                                  />
                                ) : (
